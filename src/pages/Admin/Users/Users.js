...
if(user){
  ...
  const updateRole = async () => {
    try {
      ...
      const response = await axios.put(`${process.env.REACT_APP_PATH}api/user/update`, { id: user.id, role:role})
      if(response){
        handleClose()
        console.log("updated")
      }
  ...
  async function deleteUser(id) {
    try {
      const response = await axios.delete(
        `${process.env.REACT_APP_PATH}api/user/delete`,{params: {id:id}});
      if (response) {
        console.log(response);
        getUsers();
      }
        ...
        >
          ...
          >
            ...
            >
              ...
              <form className={style.formCategory} sx={{ marginTop: '-60px !important ' }}>
                ...
                <button type="submit" className={style.addButton} onClick={updateRole} >
                  Update
                </button>
              </form>
